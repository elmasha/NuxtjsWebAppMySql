...
export default {
  ...
  methods: {
    async Fetch_PostAllEstates() {
      let that = this;
      axios
        .get(`http://localhost:5000/api/estates/estate/${that.$route.params.id}`, {})
    ...
    async Fetch_AllPayments() {
      ...
      axios
        .get(
          `http://localhost:5000/api/payments/getByEstateId/${that.$route.params.id}`,
          {}
        )
        .then(function (response) {
          if (response.status == 200) {
    ...
    async Fetch_EstateOfficials() {
      ...
      axios
        .get(
          `http://localhost:5000/api/households/getBHsHldEstId/${that.$route.params.id}`,
          {}
        )
        .then(function (response) {
          if (response.status == 200) {
